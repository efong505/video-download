...
def lambda_handler(event, context):
    ...
    try:
        ...
        print("Download complete, optimizing for streaming...")
        update_job_status(job_id, 'processing', 60)
        
        # Optimize video for progressive streaming (move moov atom to beginning)
        optimized_path = f"{output_path}.optimized.mp4"
        ffmpeg_cmd = [
            'ffmpeg',
            '-i', output_path,
        ...
        print(f"Optimizing with FFmpeg: {' '.join(ffmpeg_cmd)}")
        ...
        if ffmpeg_result.returncode == 0:
            ...
            print("Video optimized for streaming")
        else:
            print(f"FFmpeg optimization failed (using original): {ffmpeg_result.stderr}")
