# Re-optimize ALL S3 videos with FFmpeg faststart (no detection, just optimize everything)
# Simpler and more reliable - guarantees all videos get faststart
...
Write-Host "S3 Video Optimization Script (Optimize All)" -ForegroundColor Yellow
Write-Host "Adds FFmpeg faststart to ALL videos (no detection)" -ForegroundColor White
Write-Host "=" -NoNewline -ForegroundColor Cyan
Write-Host ("=" * 59) -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Note: This will optimize ALL selected videos, even if already optimized" -ForegroundColor Gray
Write-Host ""
...
foreach ($video in $filteredVideos) {
    ...
    try {
        ...
        # Optimize with FFmpeg (no detection, just optimize)
