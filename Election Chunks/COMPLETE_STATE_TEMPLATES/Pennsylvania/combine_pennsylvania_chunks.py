...
2. Save Grok's output to text files:
   ...
   - chunk2a_output.txt through chunk2o_output.txt (candidates 1-150, 10 per file)
   - chunk5a_output.txt (summary part 1)
   - chunk5b_output.txt (summary part 2)
   - chunk5c_output.txt (summary part 3)
   - chunk5d_output.txt (summary part 4)
3. Run: python combine_nj_chunks.py
"""

import os

# Read all chunk outputs
...
# Read only chunk2a (clean data)
candidates_content = read_file('chunk2a_output.txt')
candidates_content = candidates_content.replace('candidates = [', '').strip()
if candidates_content.endswith(']'):
    candidates_content = candidates_content[:-1].strip()
candidates_parts = [candidates_content]
    # if content:
...
# Write final script
output_file = os.path.join(os.path.dirname(__file__), 'upload_pennysylvania_data.py')
...
print(f"2. Run: python upload_pennysylvania_data.py")
