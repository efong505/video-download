...
def get_all_races():
    response = races_table.scan(FilterExpression='#state = :state', 
                                ...
                                ExpressionAttributeValues={':state': 'Ohio'})
    return response.get('Items', [])

def upload_candidates():
    all_races = get_all_races()
    print(f"Found {len(all_races)} Ohio races")
    
    with open('Election Data and Files/CSV files/ohio_candidates.csv', 'r', encoding='utf-8') as f:
...
if __name__ == '__main__':
    print("Uploading Ohio candidates...\n")
    upload_candidates()
    print("\nOhio candidates upload complete!")
